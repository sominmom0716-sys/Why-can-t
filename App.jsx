...
async function askClaudeJSON(system, userText, maxTokens = 1000) {
  ...
  const clean = raw.replace(new RegExp(bt+bt+bt+"json[\\s\\S]*?"+bt+bt+bt+"|"+bt+bt+bt,"g"), "").replace(/^[\s\S]*?({[\s\S]*})[\s\S]*$/, "$1").trim();
...
function WordTapPage({ words, blanks, sentence, ko, lv, playing, playAudio,
  ...
  const [pool] = useState(() => {
    const arr = words.map((w,i) => ({word:w, idx:i, distractor:false}));
  ...
  // Tap all words = correct (order doesn't matter)
  const isCorrect = order.length === words.length;
          ...
          {pool.map((item,i) => (
            <button key={i} onClick={() => addWord(item)} disabled={!!used[item.idx]}
              style={{padding:"8px 14px",borderRadius:20,
                border:"2px solid "+(used[item.idx]?"#EDE9FF":item.distractor?"#F77F0066":"#C4BEFF"),
...
function LessonFlow({ topic, onBack, goTo, skipReveal }) {
  ...
  // Lift explanation state so AI-generated explanation flows to all stages
  ...
  return (
    <div>
      ...
      {/* Progress bar */}
      <div className="mm-row" style={{ gap: 4, marginBottom: 14 }}>
        {[
          { id: "explain", label: "📖 설명" },

          { id: "worksheet", label: "📝 워크시트" },
          { id: "interview", label: "🎤 인터뷰" },
          ...(topic.context_en ? [{ id: "roleplay", label: "🎭 상황연습" }] : []),
        ].map((s, i) => {
          const stages = ["explain","worksheet","interview",...(topic.context_en?["roleplay"]:[])];
          ...
          return (
            <div key={s.id} onClick={() => setStage(s.id)}
              style={{ flex: 1, textAlign: "center", fontSize: 13, fontWeight: 700,
                ...
                background: active ? "#2D2B55" : done ? "#E8F8F4" : "#F8F8FC",
                color: active ? "#FFFFFF" : done ? "#9B8EF5" : "#3D4560",
                borderBottom: active ? "2px solid #7C6EE8" : "2px solid transparent" }}>
              {s.label}
            </div>
          );
        })}
      ...
      {stage === "explain" && (
        <ExplainStage
          topic={topic}
          preWritten={TEACHER_EXPLANATIONS[topic.id] || (
            topic.patterns?.length > 0 && topic.patterns[0].examples?.length > 0 ? {
              explanation_ko: topic.explanation_ko || (topic.label_ko + " — 이런 상황에서 쓰는 표현이에요."),
              trigger_ko: topic.patterns[0].explain_ko || "",
              patterns: topic.patterns,
              example_pairs: topic.example_pairs || topic.patterns.flatMap(p => p.examples || []),
              highlight_words: topic.highlight_words || [],
            } : null
          )}
          onExplanationReady={(exp) => setSharedExplanation(exp)}
          onNext={() => setStage("worksheet")}
          onBack={onBack}
      ...
      {stage === "worksheet" && (
        <WorksheetStage topic={topic} onNext={() => setStage("interview")} />
      )}
        {stage === "interview" && (
          <div>
            <button className="mm-btn-ghost" style={{marginBottom:10}} onClick={()=>setStage("worksheet")}>
              <ArrowLeft size={14}/> 워크시트로
            </button>
            <InterviewStage topic={topic} explanation={sharedExplanation}
              onNext={() => topic.context_en ? setStage("roleplay") : onBack()} />
          </div>
        )}
    </div>
  );
...
// Explain example row — shows Korean prompt + English sentence with listen + AIBubbleWithDrill
// ❤️ 소장하기 버튼
function SaveButton({ en, ko, type, source }) {
  ...
  useEffect(() => {
    if (!en) return;
    getWordbook()
      .then(list => setSaved(list.some(w => w.en === en)))
      .catch(() => {});
  }, [en]);
  ...
  async function toggle(e) {
    ...
    if (saved) {
      ...
      const ok = await saveToWordbook({ en, ko, type: type || "expression", source: source || "" });
      console.log("saveToWordbook result:", ok, "for:", en);
      setSaved(true);
      setJustSaved(true);
  ...
  return (
    <button onClick={toggle}
      title={saved ? "소장 취소" : "단어장에 소장하기"}
      style={{ background: "none", border: "none", cursor: "pointer", padding: "4px 6px",
        fontSize: justSaved ? 20 : 18, lineHeight: 1, flexShrink: 0, touchAction: "manipulation",
...
// ── Highlight grammar words in sentence ──
function highlightGrammar(text, words) {
  ...
  words.forEach(word => {
    const newParts = [];
    parts.forEach(part => {
      if (part.highlighted) { newParts.push(part); return; }
      ...
      if (idx >= 0) {
        ...
        newParts.push({text: part.text.slice(idx, idx + word.length), highlighted: true});
        newParts.push({text: part.text.slice(idx + word.length), highlighted: false});
      } else {
        newParts.push(part);
      }
    });
  ...
  return parts.map((p, i) => p.highlighted
    ? <mark key={i} style={{background:"#C4BEFF",color:"#2D2B55",borderRadius:4,padding:"0 2px",fontWeight:800}}>{p.text}</mark>
    : <span key={i}>{p.text}</span>
  );
}


function ExplainExampleRow({ p, revealed, highlightWords, accentColor }) {
  const [drillState, setDrillState] = useState(0);
  const color = accentColor || "#7C6EE8";
  return (
    <div style={{ opacity: revealed ? 1 : 0.15, transition: "opacity 0.4s" }}>
      <div style={{ background: "#F8F8FC", borderRadius: 10, padding: "10px 14px", marginBottom: drillState > 0 ? 6 : 0 }}>
        <div style={{ fontSize: 13, color: "#6B5B2A", fontWeight: 600, marginBottom: 6 }}>{p.ko}</div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button style={{ background: color, border: "none", borderRadius: "50%", width: 26, height: 26,
            cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
            onClick={() => { window.speechSynthesis && window.speechSynthesis.cancel(); speakText(p.en, 1.0); setDrillState(s => Math.max(s, 1)); }}>
            <Volume2 size={11} color="#fff" />
          </button>
          <span style={{ fontSize: 15, fontWeight: 700, color: "#2D2B55", flex: 1 }}>
            {highlightGrammar ? highlightGrammar(p.en, highlightWords || []) : p.en}
          </span>
          <button style={{ background: "none", border: "1px solid #C4BEFF", borderRadius: 8, padding: "2px 8px", fontSize: 11, color: "#5B5490", cursor: "pointer" }}
            onClick={() => speakText(p.en, 0.75)}>🐢</button>
          <SaveButton en={p.en} ko={p.ko} type="expression" source="설명" />
          {drillState === 0 && (
            <button style={{ fontSize: 11, padding: "3px 10px", borderRadius: 20, border: "1.5px solid #7C6EE8",
              background: "transparent", color: "#7C6EE8", cursor: "pointer", fontFamily: "'Nunito','Noto Sans KR',sans-serif", whiteSpace: "nowrap" }}
              onClick={() => setDrillState(1)}>
              안들려요
            </button>
          )}
        </div>
      ...
      {drillState >= 1 && (
        <div style={{ background: "#2D2B55", borderRadius: "0 0 10px 10px", padding: "8px 14px" }}>
          <AIBubbleWithDrill text={p.en} drillState={drillState} onDrillState={setDrillState} />
        </div>
      )}
    </div>
  );
...
function FlashcardVocab({ vocab, topicLabel, onBack }) {
  const B_TYPE = ["Nervous","Sad","Happy","Angry","Mad","Anxious","Jealous","Proud","Upset","Grateful","Lonely","Calm","Shy","Guilty","Curious","Hopeful","Miserable","Bitter","Apathetic","Envious","Comfortable","Uneasy","Cranky","Furious","Giddy"];
  const cards = vocab.filter(v => {
    if (!v.en || !v.ko) return false;
    if (v.en.startsWith("━━")) return false;
    if (v.en.startsWith("구조") || v.en.startsWith("패턴") || v.en === "") return false;
    return true;
  });
  ...
  function getColor(en) { return B_TYPE.includes(en) ? "#A78BFA" : "#7C6EE8"; }
  function getLabel(en) { return B_TYPE.includes(en) ? "형용사" : "표현"; }

  async function flip() {
    if (flipped) { setFlipped(false); setTranscript(""); setMicOn(false); return; }
    setFlipped(true);
    speakText(current.en, 0.85);
    setLoadingEx(true);
    try {
      const res = await askClaudeJSON(
        `Give one short natural example sentence using "${current.en}" (${current.ko}).
Return ONLY valid JSON: {"en:"example sentence","ko:"한국어 번역"}`,
        `Word: "${current.en}"`, 300
      );
      setExample(res);
    } catch(e) { setExample(null); }
    setLoadingEx(false);
  }

  function goNext() {
    setFlipped(false); setTranscript(""); setMicOn(false); setExample(null);
  ...
  function toggleMic() {
    ...
    setTranscript("");
    setMicOn(true);
    const r = startRecognition(t => setTranscript(t), () => setMicOn(false));
    if (r) micRef.current = r; else setMicOn(false);
  }

  if (!current) return null;
  const color = getColor(current.en);
  const pct = Math.round((idx) / cards.length * 100);

  return (
    <div>
      {onBack && (
        <button className="mm-btn-ghost" style={{ marginBottom: 10 }} onClick={onBack}>
          <ArrowLeft size={14} /> 뒤로
        </button>
      )}
      <div className="mm-card" style={{ marginBottom: 14 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          <div style={{ fontSize:13, fontWeight:700, color:"#2D2B55" }}>📚 핵심 표현 플래시카드</div>
          <div style={{ fontSize:12, color:"#9490B8" }}>{idx+1} / {cards.length}</div>
        </div>

        {/* Progress */}
        <div style={{ height:4, background:"#C4BEFF", borderRadius:2, marginBottom:14 }}>
          <div style={{ height:"100%", borderRadius:2, background:color, width:pct+"%", transition:"width 0.3s" }} />
        </div>

        {/* Card */}
        <div onClick={flipped ? goNext : flip}
          style={{ borderRadius:16, cursor:"pointer", minHeight:140, padding:"20px 16px",
            background:flipped?"#2D2B55":"#FFFFFF", border:"2px solid "+color+"44",
            display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
            gap:8, transition:"background 0.2s", userSelect:"none" }}>
          {!flipped ? (
            <div style={{ textAlign:"center" }}>
              <div style={{ fontSize:30, marginBottom:6 }}>🤔</div>
              <div style={{ fontSize:24, fontWeight:800, color:"#2D2B55" }}>{current.ko}</div>
              <div style={{ fontSize:12, color:"#9490B8", marginTop:8 }}>탭해서 영어로 보기</div>
            </div>
          ) : (
            <div style={{ textAlign:"center", width:"100%" }}>
              <div style={{ fontSize:26, fontWeight:900, color:"#B8ADFF", marginBottom:4 }}>{current.en}</div>
              <div style={{ fontSize:11, padding:"2px 10px", borderRadius:20, display:"inline-block",
                background:color+"22", color, fontWeight:700, marginBottom:6 }}>
                {getLabel(current.en)}
              </div>
              <div style={{ fontSize:13, color:"#9490B8", marginBottom:10 }}>{current.ko}</div>
              <button onClick={e => { e.stopPropagation(); speakText(current.en, 0.85); }}
                style={{ background:"#B8ADFF", border:"none", borderRadius:20, padding:"4px 14px",
                  fontSize:12, fontWeight:700, cursor:"pointer", color:"#2D2B55" }}>
                🔊 발음 듣기
              </button>
              <div style={{ fontSize:11, color:"#9490B8", marginTop:6 }}>탭하면 다음으로 →</div>
            </div>
          )}
        </div>
        ...
        {/* Example sentence */}
        {flipped && (
          <div style={{ marginTop:10, background:"#F8F8FC", borderRadius:12, padding:"10px 14px" }}>
            <div style={{ fontSize:11, color:"#9490B8", marginBottom:6 }}>💬 예문</div>
            {loadingEx ? (
              <div style={{ fontSize:12, color:"#B8ADFF" }}>예문 준비 중...</div>
            ) : example ? (
              <div>
                <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:4 }}>
                  <div style={{ fontSize:14, fontWeight:700, color:"#2D2B55", flex:1 }}>{example.en}</div>
                  <button onClick={() => speakText(example.en, 0.9)}
                    style={{ background:"none", border:"none", cursor:"pointer", fontSize:16, flexShrink:0 }}>🔊</button>
                </div>
                <div style={{ fontSize:12, color:"#5B5490" }}>{example.ko}</div>
                {/*  */}
                <div style={{ marginTop:8, borderTop:"1px dashed #C4BEFF", paddingTop:8 }}>
                  <div style={{ fontSize:11, color:"#9490B8", marginBottom:6 }}>🎤 따라 말해봐요</div>
                  <div style={{ display:"flex", gap:6 }}>
                    <button onClick={toggleMic}
                      style={{ padding:"5px 12px", borderRadius:20, border:"none", cursor:"pointer",
                        background:micOn?"#7C6EE8":"#2D2B55", color:"#fff",
                        fontWeight:700, fontSize:12, fontFamily:"'Nunito','Noto Sans KR',sans-serif" }}>
                      {micOn ? "⏹ 중지" : "🎤 말하기"}
                    </button>
                    {transcript && (
                      <div style={{ fontSize:12, color:"#2D2B55", padding:"5px 10px",
                        background:"#fff", borderRadius:20, flex:1, border:"1px solid #C4BEFF" }}>
                        {transcript}
                      </div>
                    )}
                  </div>
        ...
        <div style={{ display:"flex", gap:8, marginTop:12 }}>
          <button onClick={goPrev} disabled={idx === 0}
            style={{ flex:1, padding:"10px", borderRadius:12, border:"1.5px solid #C4BEFF",
              background:"#fff", cursor:idx===0?"not-allowed":"pointer",
              color:idx===0?"#C4BEFF":"#2D2B55", fontWeight:700, fontSize:13,
              fontFamily:"'Nunito','Noto Sans KR',sans-serif", opacity:idx===0?0.4:1 }}>
            ← 이전
          </button>
          <button onClick={goNext} disabled={idx === cards.length - 1}
            style={{ flex:1, padding:"10px", borderRadius:12, border:"none",
              background:idx===cards.length-1?"#C4BEFF":"#7C6EE8",
...
// ── 쌤 가상 선생님 — 칠판 위를 걸어다니며 설명 ──
function TeacherAvatar({ slides, patterns, onClose, onCurChange }) {
  ...
  const steps = [
    { patternIdx: -1, leftPct: 5, arm: 'wave' },
    ...patterns.map((_, i) => ({
      patternIdx: i,
      leftPct: Math.min(10 + i * 20, 65),
      arm: i % 2 === 0 ? 'point' : 'wave',
    })),
    { patternIdx: -1, leftPct: 80, arm: 'wave' },
  ];
  ...
  const allSlides = [
    cleanText(slides[0]) || "안녕하세요! 오늘 같이 배워봐요!",
    ...patterns.map((p, i) => cleanText(slides[i+1]) || cleanText(p.pattern_en + (p.explain_ko ? ". " + p.explain_ko : ""))),
    cleanText(slides[slides.length-1]) || "자 이제 패턴들을 직접 연습해봐요!",
  ];

  useEffect(() => {
    speakText(allSlides[0], 0.9);
    setSpeaking(true);
  ...
  function goTo(i) {
    ...
    setCur(i);
    if (onCurChange) onCurChange(i);
    setWalking(true);
    setTimeout(() => setWalking(false), 700);
    setMarked(steps[i]?.patternIdx ?? -1);
    window.speechSynthesis?.cancel();
    setSpeaking(true);
    const u = new SpeechSynthesisUtterance(allSlides[i]);
  ...
  function goNext() {
    if (cur < allSlides.length - 1) goTo(cur + 1);
    else { window.speechSynthesis?.cancel(); onClose(); }
  }

  const step = steps[cur] || steps[0];
  const leftPct = step.leftPct;
  const isPoint = step.arm === 'point';

  return (
    <div ref={boardRef} style={{ background:"#1E3D2F", borderRadius:16,
      border:"4px solid #152D22", marginBottom:14, overflow:"hidden",
      position:"relative" }}>

      {/* Chalk texture lines */}
      <div style={{ position:"absolute", inset:0, opacity:0.03,
        backgroundImage:"repeating-linear-gradient(transparent,transparent 28px,#fff 28px,#fff 29px)",
        pointerEvents:"none" }} />
      ...
      <div style={{ padding:"14px 14px 8px" }}>
        ...
        {patterns.map((p, pi) => (
          <div key={pi} style={{ borderRadius:10, padding:"10px 12px", marginBottom:8,
            border: marked===pi ? "1.5px solid #F2D76B" : "1.5px solid rgba(255,255,255,0.08)",
            background: marked===pi ? "rgba(242,215,107,0.08)" : "transparent",
            transition:"all 0.4s", position:"relative" }}>
            <div style={{ fontSize:14, fontWeight:800, color:"#F0EEFF" }}>{p.pattern_en}</div>
            {p.explain_ko && <div style={{ fontSize:11, color:"rgba(255,255,255,0.45)", marginTop:3 }}>{p.explain_ko}</div>}
            {/* Underline mark */}
            {marked===pi && (
              <div style={{ height:2.5, borderRadius:2, background:"#F2D76B",
                marginTop:6, animation:"drawLine 0.5s ease forwards",
                transformOrigin:"left" }} />
            )}
            {/* Pointer arrow */}
            {marked===pi && (
              <div style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)",
                fontSize:18, animation:"pointPulse 0.6s ease-in-out infinite alternate" }}>
                👈
              </div>
            )}
          </div>
      ...
      {/* Teacher character */}
      <div style={{ position:"relative", height:120, margin:"0 0 0" }}>
        {/* Speech bubble */}
        <div style={{ position:"absolute", bottom:90, left:`${Math.min(leftPct, 55)}%`,
          background:"#fff", borderRadius:"12px 12px 12px 4px",
          padding:"8px 12px", maxWidth:180, fontSize:11, fontWeight:700,
          color:"#2D2B55", lineHeight:1.5, zIndex:2,
          transition:"left 0.7s cubic-bezier(0.34,1.4,0.64,1)" }}>
          {allSlides[cur]}
          {speaking && (
            <div style={{ display:"flex", gap:2, alignItems:"center", marginTop:4 }}>
              {[5,10,7,12,5].map((h,i) => (
                <div key={i} style={{ width:3, height:h, borderRadius:2, background:"#7C6EE8",
                  animation:`wv 0.7s ${i*0.1}s ease-in-out infinite` }} />
              ))}
            </div>
          )}
        ...
        <div style={{ position:"absolute", bottom:0,
          left:`${leftPct}%`, transition:"left 0.7s cubic-bezier(0.34,1.4,0.64,1)",
          display:"flex", flexDirection:"column", alignItems:"center" }}>
          {/* Head */}
          <div style={{ width:42, height:42, borderRadius:"50%",
            background:"#F9C784", border:"2.5px solid #E8A84C",
            display:"flex", alignItems:"center", justifyContent:"center",
            fontSize:20, position:"relative",
            animation:"headBob 1.8s ease-in-out infinite" }}>
            <div style={{ position:"absolute", top:-4, left:"50%", transform:"translateX(-50%)",
              width:44, height:22, background:"#4A2E00", borderRadius:"22px 22px 0 0" }} />
            😊
            {/* Pointing arm */}
            <div style={{ position:"absolute", right:-22, top:14,
              width:24, height:7, background:"#7C6EE8", borderRadius:4,
              transform: isPoint ? "rotate(-40deg)" : "rotate(-15deg)",
              transformOrigin:"left center",
              transition:"transform 0.4s",
              animation: !isPoint ? "armWave 0.5s ease-in-out infinite alternate" : "none" }}>
              <span style={{ position:"absolute", right:-8, top:-4, fontSize:13 }}>👆</span>
            </div>
          </div>
          {/* Body */}
          <div style={{ width:34, height:36, background:"#7C6EE8",
          ...
          <div style={{ display:"flex", gap:4 }}>
            {[0, 0.2].map((delay, i) => (
              <div key={i} style={{ width:11, height:20, background:"#3D2B7C",
                borderRadius:"0 0 6px 6px",
                animation: walking ? `walk 0.3s ${delay}s ease-in-out infinite alternate` : "none" }} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8,
        ...
        <div style={{ display:"flex", gap:5, flex:1 }}>
          {allSlides.map((_, i) => (
            <div key={i} onClick={() => goTo(i)}
              style={{ height:6, borderRadius:3, cursor:"pointer", transition:"all 0.2s",
                width: i===cur ? 20 : 6,
                background: i===cur ? "#B8ADFF" : "rgba(255,255,255,0.2)" }} />
          ))}
        </div>
        <button onClick={() => { window.speechSynthesis?.cancel(); onClose(); }}
          style={{ background:"rgba(255,255,255,0.1)", border:"1.5px solid rgba(255,255,255,0.2)",
            color:"rgba(255,255,255,0.6)", borderRadius:20, padding:"5px 12px",
            fontSize:11, fontWeight:800, cursor:"pointer",
            fontFamily:"'Nunito','Noto Sans KR',sans-serif" }}>
          닫기
        </button>
        <button onClick={goNext}
          style={{ background:"#B8ADFF", border:"none", color:"#2D2B55",
            borderRadius:12, padding:"7px 14px", fontSize:12, fontWeight:800,
            cursor:"pointer", fontFamily:"'Nunito','Noto Sans KR',sans-serif" }}>
          {cur === allSlides.length-1 ? "완료 ✓" : "다음 →"}
        </button>
      </div>

      <style>{`
        @keyframes headBob{0%,100%{transform:rotate(-3deg)}50%{transform:rotate(3deg)}}
        @keyframes armWave{from{transform:rotate(-15deg)}to{transform:rotate(-50deg)}}
        @keyframes walk{from{transform:rotate(8deg)}to{transform:rotate(-8deg)}}
        @keyframes pointPulse{from{transform:translateY(-50%)}to{transform:translateY(-50%) translateX(-3px)}}
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are an English coach for Korean learners. Analyse the sentence and explain why it sounds hard to hear.

Output ONLY this exact JSON structure (no markdown, no explanation):
{
  "ko": "Korean translation of the full sentence",
  "words": [
    {
      "word": "key word or chunk (2-4 words)",
      "type": "liaison",
      "ko_meaning": "Korean meaning",
      "detail_ko": "Why this is hard to hear in Korean",
      "sounds_like": "how it sounds when spoken fast (or empty string)",
      "example_en": "another example sentence",
      "example_ko": "Korean translation"
    }
  ],
  "grammar_note": "Key grammar point in Korean",
  "usage_note": "When to use this in Korean"
}

Rules:
- Include exactly 3 words/chunks
- type must be one of: liaison, expression, grammar
- For liaison: explain connected speech (e.g. "want to" sounds like "wanna")
- For expression: explain meaning and usage
- For grammar: explain why this tense/structure is used
- All detail_ko and grammar_note must be in Korean
- sounds_like should show fast speech pronunciation`,
          messages: [{ role: "user", content: `Analyse this sentence: "${sentence}"` }],
        }),
      });
      const data = await res.json();
      const raw = (data.content || []).map(b => b.text || "").join("");
      ...
      try {
        const res2 = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: `Analyse this English sentence for Korean learners. Return JSON:
{"ko":"Korean translation","words":[{"word":"key chunk","type":"expression","ko_meaning":"meaning in Korean","detail_ko":"explanation in Korean","sounds_like":"fast pronunciation","example_en":"example","example_ko":"Korean example"}],"grammar_note":"grammar in Korean","usage_note":"usage in Korean"}

Sentence: "${sentence}"
Include 3 words. Respond with JSON only.` }],
          }),
        });
        const data2 = await res2.json();
        const raw2 = (data2.content || []).map(b => b.text || "").join("");
  ...
  return (
    <div style={{ background: "#0E1628", borderRadius: 10, padding: "12px 14px", marginBottom: 8 }}>
      <div style={{ fontSize: 11, color: "#7C6EE8", fontWeight: 700, marginBottom: 8 }}>2차 안들려요 — 왜 안들리는지 알아봐요</div>

      {/* Highlighted sentence */}
      <div style={{ fontSize: 15, fontWeight: 700, lineHeight: 2, marginBottom: 8 }}>
        {(() => {
          if (!annotation?.words?.length) return <span style={{ color: "#FFFFFF" }}>{sentence}</span>;
          let parts = [{ text: sentence, isWord: false }];
          [...(annotation.words)].sort((a,b) => sentence.toLowerCase().indexOf(a.word.toLowerCase()) - sentence.toLowerCase().indexOf(b.word.toLowerCase()))
            .forEach(w => {
              const newParts = [];
              parts.forEach(part => {
                if (part.isWord) { newParts.push(part); return; }
                const idx = part.text.toLowerCase().indexOf(w.word.toLowerCase());
                if (idx >= 0) {
                  if (idx > 0) newParts.push({ text: part.text.slice(0, idx), isWord: false });
                  newParts.push({ text: part.text.slice(idx, idx + w.word.length), isWord: true, word: w });
                  newParts.push({ text: part.text.slice(idx + w.word.length), isWord: false });
                } else { newParts.push(part); }
              });
              parts = newParts;
            });
          return parts.map((p, i) => {
            if (!p.isWord) return <span key={i} style={{ color: "#FFFFFF" }}>{p.text}</span>;
            const color = TYPE_COLORS[p.word.type] || "#B8B4D8";
            return (
              <span key={i} style={{ color, background: color + "25", borderRadius: 4, padding: "0 2px",
                textDecoration: p.word.type === "liaison" ? "underline wavy" : "underline solid",
                textDecorationColor: color, textDecorationThickness: 2, cursor: "pointer" }}
                title={p.word.ko_meaning}
                onClick={() => speakText(p.word.word, 0.75)}>
                {p.text}
              </span>
            );
          });
      ...
      {annotation?.words?.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 10 }}>
          {[...new Set(annotation.words.map(w => w.type))].map(type => {
            const c = TYPE_COLORS[type] || "#B8B4D8";
            return <span key={type} style={{ fontSize: 10, fontWeight: 700, color: c, background: c+"22", padding: "1px 7px", borderRadius: 20, border: `1px solid ${c}44` }}>{TYPE_LABELS[type]}</span>;
          })}
        </div>
      )}

      {/* Korean translation */}
      {annotation?.ko && <div style={{ fontSize: 12, color: "#8C9AB8", marginBottom: 10, background: "rgba(255,255,255,0.05)", borderRadius: 6, padding: "5px 8px" }}>💬 {annotation.ko}</div>}

      {/* Word cards */}
      {annotation?.words?.map((w, i) => {
        const color = TYPE_COLORS[w.type] || "#B8B4D8";
        return (
          <div key={i} style={{ marginBottom: 8, borderRadius: 8, background: color+"14", border: `1.5px solid ${color}44`, overflow: "hidden" }}>
            <div style={{ background: color+"22", padding: "6px 10px", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <button onClick={() => speakText(w.word, 0.75)}
                style={{ background: color, border: "none", borderRadius: "50%", width: 22, height: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <Volume2 size={10} color="#fff" />
              </button>
              <span style={{ fontWeight: 800, fontSize: 14, color: "#FFFFFF", textDecoration: "underline", textDecorationColor: color }}>{w.word}</span>
              {w.sounds_like && <span style={{ fontSize: 11, background: "#B8ADFF", color: "#2D2B55", padding: "1px 7px", borderRadius: 20, fontWeight: 900 }}>🔗 "{w.sounds_like}"</span>}
              <span style={{ marginLeft: "auto", fontSize: 10, color, background: color+"25", padding: "1px 6px", borderRadius: 10 }}>{TYPE_LABELS[w.type]}</span>
            </div>
            <div style={{ padding: "6px 10px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#FFFFFF", marginBottom: 2 }}>{w.ko_meaning}</div>
                <SaveButton en={w.word} ko={w.ko_meaning} type="collocation" source="인터뷰 분석" />
              </div>
              {w.detail_ko && <div style={{ fontSize: 12, color: "#B8B4D8" }}>{w.detail_ko}</div>}
              {w.collocation && <div style={{ fontSize: 11, color, marginTop: 4, background: color+"12", borderRadius: 5, padding: "2px 6px", display: "inline-block" }}>💡 {w.collocation}</div>}
              {w.example_en && (
                <ExampleToggle word={w.word} example_en={w.example_en} example_ko={w.example_ko} color={color} />
              )}
            </div>
          </div>
        );
      ...
      {annotation?.grammar_note && (
        <div style={{ marginBottom: 6, padding: "8px 10px", background: "rgba(155,89,182,0.15)", borderRadius: 6, borderLeft: "3px solid #6C5CE7" }}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
            <div>
              <span style={{ fontSize: 11, fontWeight: 800, color: "#6C5CE7" }}>📐 문법 </span>
              <span style={{ fontSize: 12, color: "#C8D8E8" }}>{annotation.grammar_note}</span>
              {annotation.usage_note && (
                <div style={{fontSize:11,color:"#9490B8",marginTop:4}}>{annotation.usage_note}</div>
              )}
            </div>
            <GrammarSaveButton sentence={sentence} annotation={annotation} />
          </div>
        </div>
      )}

      {/* Practice buttons */}
      <div style={{ marginTop: 10, paddingTop: 8, borderTop: "1px dashed rgba(255,255,255,0.1)", display: "flex", gap: 6, flexWrap: "wrap" }}>
        <div style={{ fontSize: 11, color: "#9490B8", width: "100%", marginBottom: 4 }}>연습하기</div>
        <button className="mm-btn-ghost" style={{ fontSize: 11 }} onClick={() => speakText(sentence, 1.0)}>🔊 다시 듣기</button>
        <SlowSpeedToggle onPlay={(speed) => speakText(sentence, speed)} />
        {SpeechRec && (
          <button className={"mm-btn-ghost " + (micOn ? "mm-mic-active" : "")} style={{ fontSize: 11 }} onClick={toggleMic}>
            {micOn ? <MicOff size={10} /> : <Mic size={10} />} 따라 말하기
          </button>
        )}
        {annotation?.words?.some(w => w.type === "liaison") && (
          <button className="mm-btn-ghost" style={{ fontSize: 11 }} onClick={() => {
            const liaisonWord = annotation.words.find(w => w.type === "liaison");
            if (liaisonWord?.sounds_like) speakText(liaisonWord.sounds_like, 0.75);
          }}>🔗 연음만 연습</button>
        )}
      </div>
...
function SimilarSentencesDrill({ sentence, annotation }) {
  ...
  async function load() {
    ...
    try {
      const res = await askClaudeJSON(
        `Generate 4 natural English sentences similar in difficulty to the example.
Return ONLY valid JSON:
{"sentences":[{"en":"sentence here","ko":"한국어번역"},{"en":"...","ko":"..."},{"en":"...","ko":"..."},{"en":"...","ko":"..."}]}
8-12 words each. Natural spoken English.`,
        `Original: "${sentence}"`
      );
...
{"question":"...","options":[{"text":"1-2 words","tier":"perfect"},{"text":"1-2 words","tier":"okay"},{"text":"1-2 words","tier":"wrong"}]}`,
      ...
      if (r?.question && Array.isArray(r?.options) && r.options.length >= 3) {
        ...
        setCompQ({
          ...
          options: [
            {text: "The main idea of the sentence", tier: "perfect"},
            {text: "Something related but not exact", tier: "okay"},
            {text: "Something completely different", tier: "wrong"},
